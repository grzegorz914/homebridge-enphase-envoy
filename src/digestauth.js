        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if (resError === undefined || resError.status !== 401) {
                    reject(`Digest authentication response error: ${resError}`);
                    return;
                };

                const resHeaders = resError.headers["www-authenticate"];
                if (!(resHeaders === null || resHeaders === void 0 ? void 0 : resHeaders.includes('nonce'))) {
                    reject(`Digest authentication headers error: ${resError.headers}`);
                    return;
                };
